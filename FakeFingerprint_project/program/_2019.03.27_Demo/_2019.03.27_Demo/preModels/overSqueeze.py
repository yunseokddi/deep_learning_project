...
def fire_module2(x, fire_id, squeeze=16, expand=64, dim_ordering='tf'):
    s_id = 'fire'+str(fire_id)+'/'
    if dim_ordering is 'tf':
        c_axis = 3
    else:
...
def shirinking_squeezenet(weights_path=None, heatmap=False, nb_classes = 3, input_size=32):
    ...
    # x = Activation('relu', name='relu_conv1')(x)
    x = fire_module2(x, fire_id=2, squeeze=16, expand=64)
    # 32: 15x15, 23x23, 31x31
    x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool1')(x) 
    x = fire_module2(x, fire_id=3, squeeze=16, expand=64)
    x = fire_module2(x, fire_id=4, squeeze=32, expand=128)
    # x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool4')(x) 
    x = fire_module2(x, fire_id=5, squeeze=32, expand=128)
    x = fire_module2(x, fire_id=6, squeeze=48, expand=192)
    # 32: 7x7, 11x11, 15x15
    x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool6')(x) 
    x = fire_module2(x, fire_id=7, squeeze=48, expand=192)
    x = fire_module2(x, fire_id=8, squeeze=64, expand=256)
    # x = fire_module2(x, fire_id=8, squeeze=64, expand=256)
    x = Dropout(0.5, name='drop7')(x)    
    x = Conv2D(nb_classes, (1, 1), padding='valid', name='conv7')(x)
    x = BatchNormalization(name='batch_conv7')(x)
    # x = Activation('relu', name='relu_conv10')(x)
    x = LeakyReLU(name='leaky_conv7')(x)
    if heatmap:
...
def convnet(network, weights_path=None, heatmap=False,input_size=32, nb_classes=3):
    ...
    * For VGG16 and VGG19, the data are of shape (224,224), and the colors in the BGR order
    ...
    if network == "basic":
        ...
        convnet_init = shirinking_squeezenet2
    else:
